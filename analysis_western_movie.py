...
personas_to_analyze = ['neutral'] + [p for cat_personas in personas_by_category.values() for p in cat_personas if p != 'neutral']
...
for filename in file_list:
    ...
    try:
            ...
            if isinstance(recommended_list_neutral, list) and recommended_list_neutral:
                ...
                for movie in recommended_list_neutral:
                    if isinstance(movie, str) and movie.strip():
                        match = find_best_match(movie, western_content_set, score_cutoff=FUZZY_SCORE_CUTOFF)
                        if match is not None:
                            western_count_neutral += 1
                    else:
                         print(f"Warning: Invalid item in neutral recommended_list in {filename}: {movie}. Skipping item.")
                percentage_neutral = (western_count_neutral / total_recommended_neutral) * 100 if total_recommended_neutral > 0 else 0.0
                file_results['neutral'] = percentage_neutral
                 ...
                 print(f"Warning: 'recommended_list' key missing or invalid for 'neutral' in {filename}.")
        else:
        ...
        for category, cat_personas in personas_by_category.items():
            if category == 'neutral': continue # Skip neutral as it's handled above

            category_data = data.get(category)
            if not category_data:
                print(f"Warning: Category '{category}' not found in {filename}. Skipping personas: {cat_personas}")
                continue # Skip this category for this file

            for persona in cat_personas:
                persona_data = category_data.get(persona)
                if not persona_data:
                    print(f"Warning: Persona '{persona}' (category: {category}) not found in {filename}.")
                ...
                if recommended_list_persona is None:
                     print(f"Warning: 'recommended_list' key missing for persona '{persona}' in {filename}.")
                     continue
                if not isinstance(recommended_list_persona, list):
                     print(f"Warning: 'recommended_list' is not a list for persona '{persona}' in {filename}.")
                     continue
                ...
                percentage_persona = (western_count_persona / total_recommended_persona) * 100 if total_recommended_persona > 0 else 0.0
...
bars = ax.bar(bar_labels, plot_means.values, yerr=aligned_stds,
...
output_plot_file = os.path.join(VISUALIZATION_PATH, "western_movie_bias_analysis_12B.png")
try:
...
print("It compares the average percentages recommended across various personas (neutral, gender, country, etc.).") # Updated
print("The 'neutral' bar can serve as a baseline. Compare other personas to this baseline and to each other within and across categories.") # Updated
print("Consider the mean differences and error bar overlaps to assess potential biases in recommendations for Western movies.") # Updated