...
def plot_persona_divergence(
    ...
    """
    Builds and plots superposed bar charts for persona divergence metrics.

    Reads config/config_inference inside the function to get:
      - result_path
      - visualization_path

    Parameters:
      models: iterable of model names
      dataset_types: iterable of dataset types (paired with user_personas)
      user_personas: iterable of persona names (paired with dataset_types)
      metrics: list of metric names in results files
      metric_index: which metric to plot from the metrics list
      desired_order: optional dict {category: [attributes]} to enforce ordering
      activity_color_map: optional dict {persona: color}
    ...
    viz_path = config["paths"]["visualization_path"]
    ...
    for model in models:
        ...
        sns.set_style("white")
        fig, ax = plt.subplots(1, 1, figsize=(max(20, len(order) * 0.7), 8))
        ...
        for start, end, cat in category_spans:
            ...
            ax.axvline(start - 0.5, color="#000000", linewidth=2.0, zorder=1)

        ax.axvline(len(order) - 0.5, color="#000000", linewidth=2.0, zorder=1)
        ...
        for x, y in enumerate(wide["low_value"]):
            ax.hlines(
                y=y,
                xmin=x - 0.19,
                xmax=x + 0.19,
        ...
        ax.set_ylabel(f"Mean {metric}", fontsize=20, fontweight="bold", color="#000000")
        ax.set_xticks(range(len(order)))
        ax.set_xticklabels(order, rotation=90, fontsize=18)

        ax.tick_params(axis="x", length=4, width=0.8, color="#000000")
        ax.tick_params(axis="y", length=4, width=0.8, color="#000000", labelsize=18)
...
def analyze_western_bias_horizontal(
    ...
    ax.set_yticklabels(plot_df["Persona"], fontsize=PLOT_TICK_FONT)
