...
ordered_model_names = [model_names_map[model_id] for model_id in model_ids] # For consistent column order
...
def extract_iou_means_flat(filename, attributes_to_extract):
    ...
    try:
        ...
        for attribute in attributes_to_extract:
            ...
            # *** Key for IOU Divergence ***
            iou_dict = attr_data.get('IOU Divergence') # Assuming this key structure
...
for dataset in datasets:
    ...
    for model_id in model_ids:
        ...
        if iou_means_dict is None: # Check for file not found return value
             print(f"    -> Error processing file (likely not found, handled above).")
             overall_mean_ious[dataset.capitalize()][model_display_name] = np.nan
             # all_files_found is already False from the os.path.exists check
             continue
        elif not iou_means_dict: # Check for empty dict return value (JSON error, etc.)
        ...
        if valid_iou_values:
            ...
            # This case handles when the file exists and is valid JSON, but no 'IOU Divergence'->'mean' keys were found
...
ax.set_ylabel("Overall Mean IOU Divergence", fontsize=14) # Increased fontsize
ax.set_title("Overall Mean IOU Divergence: Gemma 3 4B vs Llama 3.2 3B Across Datasets", fontsize=16, pad=15) # Increased fontsize
ax.tick_params(axis='x', rotation=0, labelsize=12) # Keep dataset names horizontal, increased labelsize
ax.tick_params(axis='y', labelsize=12) # Increased y-axis tick labelsize
ax.grid(axis='y', linestyle='--', alpha=0.7)
ax.legend(title="Model", fontsize=12, title_fontsize=13) # Increased legend font sizes
...
if not df_plot.dropna().empty:
    ...
    if pd.notna(min_val) and pd.notna(max_val):
         ...
         padding = (max_val - min_val) * 0.1 if max_val > min_val else 0.1 # Handle case where min=max
         ax.set_ylim(bottom=max(0, min_val - padding),
                     top=max_val + padding)
    elif pd.notna(max_val): # Handle case where only max_val is valid (e.g., all NaNs except one value)
...
# --- Save and Show Plot ---
output_plot_file = os.path.join(VISUALIZATION_PATH, "overall_iou_divergence_gemma_vs_llama.png")
...
print("\n--- Interpretation ---")
print("This bar chart compares the average 'IOU Divergence' across all sensitive attributes for Gemma 3 4B and Llama 3.2 3B models, grouped by dataset.")
print("Each group on the x-axis represents a dataset (College, Music, Movie).")
print("Within each group, the bars represent the Gemma 3 4B and Llama 3.2 3B models.")
