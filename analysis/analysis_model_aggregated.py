...
def extract_metric_means_flat(
    filename, attributes_to_extract, metric_name="IOU Divergence"
):
    """
    Loads JSON data and extracts the mean metric_name for specified attributes.
    """
    iou_data = {}
    try:
        ...
        for attribute in attributes_to_extract:
            ...
            # *** UPDATED KEY HERE ***
            iou_dict = attr_data.get(metric_name)
        ...
        if extracted_count == 0:
            print(
                f"  Warning: No {metric_name} -> 'mean' values found in {os.path.basename(filename)} for the requested attributes."
            )
...
# --- Load Data and Calculate Overall Mean metric_name Divergence ---
overall_mean_ious = {}
...
for metric in metrics:
    for dataset in datasets:
        ...
        for model_size in model_sizes:
            ...
            # Extract metric_name means for all attributes for this specific file
            iou_means_dict = extract_metric_means_flat(
                filename, attribute_order, metric_name=metric
            )

            if (
                iou_means_dict is None
            ):  # Specific check for file not found from function
                overall_mean_ious[dataset.capitalize()][model_display_name] = np.nan
    ...
    ax.set_ylabel(f"Overall Mean {metric}", fontsize=18)  # Increased fontsize
    ax.set_title(
        f"Overall Mean {metric} per Model Across Datasets", fontsize=20, pad=15
    )  # Increased fontsize
    ax.tick_params(
        axis="x", rotation=0, labelsize=16
    ...
    ax.legend(
        title="Model Size", fontsize=16, title_fontsize=17
    ...
    if not df_plot.dropna().empty:
        ...
        if pd.notna(min_val) and pd.notna(max_val):
            ax.set_ylim(
                bottom=max(0, min_val - (max_val - min_val) * 0.1),
    ...
    output_plot_file = os.path.join(
        VISUALIZATION_PATH, "overall_iou_divergence_by_model_dataset.png"
    )
