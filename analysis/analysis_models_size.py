...
# --- Configuration ---
config rel= configparser.ConfigParser()
...
# --- Gemma Models (Original Configuration) ---
# file_mapping = {
...
# --- Qwen Models (Current Configuration) ---
file_mapping = {
    "Qwen 14B": os.path.join(RESULT_PATH, "qwen3_14b_music_.json"),
    "Qwen 8B": os.path.join(RESULT_PATH, "qwen3_8b_music_.json"),
    "Qwen 4B": os.path.join(RESULT_PATH, "qwen3_4b_music_.json"),
    "Qwen 1.7B": os.path.join(RESULT_PATH, "qwen3_1.7b_music_.json"),
    "Qwen 0.6B": os.path.join(RESULT_PATH, "qwen3_0.6b_music_.json"),
}
                ...
                if isinstance(iou_dict, dict):
                    ...
                    if mean_val is not None:
                        ...
                        try:
                            ...
                            print(
                                f"Warning: Could not convert mean IOU '{mean_val}' to float for attribute '{attribute}' in {os.path.basename(filename)}. Setting to NaN."
                            )
                            mean_iou = np.nan  # Ensure it's NaN if conversion fails

            iou_data[attribute] = (
                mean_iou  # Store NaN if any step failed or value wasn't found/valid
            )

    except FileNotFoundError:
        print(f"Error: File not found: {filename}")
        return {
            key: np.nan for key in attributes_to_extract
        }  # Return NaNs for all requested keys
...
if final_rows == 0:
    ...
    print(
        "This likely means the extraction function failed to find 'IOU'->'mean' even with the corrected logic,"
    )
    print(
        "or the files have significantly different attribute sets / missing IOU blocks."
    )
    print("Please double-check:")
    print(
        "  1. The exact file paths and names (especially 'gemma3_college_.json' for 4B)."
    )
    print(
        "  2. The JSON structure within EACH file *exactly* matches the provided samples (top-level attribute keys)."
    )
    print(
        "  3. The attribute keys in the JSON match the strings in attribute_order list (case-sensitive)."
    )
...
bar_width = 0.18  # Adjusted for 5 bars
...
ax.set_title(
    "Comparison of Mean IOU Score Across Qwen Models by Size (Music Domain)",
    fontsize=14,
    pad=20,
)  # Updated title
...
ax.set_xticklabels(
    df_plot.index, rotation=60, ha="right", fontsize=9
)  # Changed rotation
...
if not df_plot.empty:
    ...
    ax.set_ylim(
        bottom=max(0, min_val - (max_val - min_val) * 0.05),
        top=max_val + (max_val - min_val) * 0.1,
    )  # Extend top limit for label space

# --- Save and Show Plot ---
os.makedirs(visualization_dir, exist_ok=True)
output_plot_file = os.path.join(
    visualization_dir, "iou_comparison_qwen_models_music.png"
)
...
print("\n--- Interpretation ---")
print(
    "This grouped bar chart compares the average IOU scores for different Qwen models across various attributes."
)
print(
    "Each group represents an attribute; bars within a group show scores for different Qwen model sizes."
)
print(
    "Compare bar heights within groups to see how IOU varies with model size for each attribute."
)
print(
    "Attributes missing data in *any* of the compared files are excluded from this plot."
)
